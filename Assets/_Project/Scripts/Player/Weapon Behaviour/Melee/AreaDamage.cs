...
namespace BadJuja.Player.Weapons.Behaviour {
    public class AreaDamage : IWeaponBehaviour {
        ...
        public AreaDamage(PlayerWeaponMelee weaponScript, float radius, float attackInterval)
        {
        ...
        private void TriggerAttack()
        {
            ...
            {
                if (enemy.TryGetComponent(out IDamagable damagable) && enemy.TryGetComponent(out IStats stats))
                {
                    _parentWeapon.DealDamage(damagable, stats, enemies.Length);
                }
            }
        }
