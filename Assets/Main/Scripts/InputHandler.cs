...
public class InputHandler : MonoBehaviour {
    ...
    private void Send_SwitchToAttackType(int value)
    {
        Send_OnSwitchToAttackType(value);
    }

    private void OnEnable()
    {
        _switchToMeleeAction.performed += ctx => Send_SwitchToAttackType(0);
        _switchToLightAction.performed += ctx => Send_SwitchToAttackType(1);
        _switchToHeavyAction.performed += ctx => Send_SwitchToAttackType(2);
    }
